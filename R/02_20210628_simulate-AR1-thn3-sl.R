...
for (i in seq_along(ts.mcem)) {
  ts.mcem[[i]] = twoStageMCEM(formula.mu=formula.mu, formula.phi=formula.phi, formula.p=formula.p, 
                              df=df.ar1[[i]][,-3], x=c("x1", "x2"), timepoints="tp", AR=AR, 
...
# save data
saveRDS(df.ar1, file="20210916-simData-ar1-thetan3-sigma-l.RDS")
saveRDS(ts.mcem, file="20210916-tsmcem-ar1-thetan3-sigma-l.RDS")